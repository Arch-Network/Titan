...
impl FromStr for Rune {
    ...
    fn from_str(s: &str) -> std::result::Result<Self, RuneError> {
        let mut x = 0u128;
        for (i, c) in s.chars().enumerate() {
            if i > 0 {
                x = x.checked_add(1).ok_or(RuneError::Range)?;
            }
            x = x.checked_mul(26).ok_or(RuneError::Range)?;
            match c {
                'A'..='Z' => {
                    x = x
                        .checked_add(c as u128 - 'A' as u128)
                        .ok_or(RuneError::Range)?;
                }
                _ => return Err(RuneError::Character(c)),
            }
        }
        Ok(Rune(x))
    }
...
mod tests {
    ...
    fn from_str_error() {
        ...
        assert_eq!("x".parse::<Rune>().unwrap_err(), RuneError::Character('x'));
    }
    ...
    fn borsh_roundtrip() {
        let rune = Rune(42);
        let serialized = borsh::to_vec(&rune).unwrap();
        let deserialized: Rune = borsh::from_slice(&serialized).unwrap();
        assert_eq!(rune, deserialized);
    }
}
